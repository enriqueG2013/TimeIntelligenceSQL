...
/* Tables and certain attributes names were changed dor work confidential */
...
Nullif(LAG(Amount) over (order by Year(fiscalperiodName), Month(fiscalPeriodName)), 0 ))

LAG(Amount) over (order by Year(fiscalperiodName), Month(fiscalPeriodName)) as MoM,
Amount  - (LAG(Amount) over (order by Year(fiscalperiodName), Month(fiscalPeriodName))) as MoMvar,
((Amount  - LAG(Amount) over (order by Year(fiscalperiodName), Month(fiscalPeriodName))) /
Nullif(LAG(Amount) over (order by Year(fiscalperiodName), Month(fiscalPeriodName)), 0 )) * 100 as MoMper,
--LAG(QuarterAmount) over (Partition by a.FiscalYearName order by a.FiscalQuarter) as QoQ,

sum(Amount) - Lag(sum(Amount), 11) over (order by Year(fiscalperiodName), Month(fiscalperiodName)) as YoYvar,
((sum(Amount) - Lag(sum(Amount), 11) over (order by Year(fiscalperiodName), Month(fiscalperiodName))) /
Lag(sum(Amount), 11) over (order by Year(fiscalperiodName), Month(fiscalperiodName))) * 100 as YoY_Var_Perentage


((sum(Amount) over (partition by FiscalQuarter order by FiscalPeriod)) - coalesce(lag(QTD) over (order by FiscalYearname), 0) /
	nullif(coalesce(lag(QTD) over (order by FiscalYearname), 0),0)) * 100 as QTD_Var_Percentage